...
export class SubscriptionMethod extends BaseMethod {
  ...
  public async getByAddress(context: MethodContext, address: Buffer): Promise<GetByAddressResult> {
    ...
    const account = await subscriptionAccountStore.get(
      context,
      address,
    );
  ...
  public async consume(context: MethodContext, subscriptionID: Buffer, address: Buffer): Promise<void> {
    ...
    const subscription = await subscriptionStore.get(
      context,
      subscriptionID,
    );
